...
const dummyLeaderboardBlocks = [
  {
    ...
    template: [
      ...
      '        ',
      '        <div class="leaderboard-row relative group" style="animation-delay: {{ row_delay }}s;">',
      '          ',
      '          {% if contributor.rank == 1 %}',
      '          <div class="border-[4px] border-black px-4 py-2.5 relative overflow-hidden">',
      '            <div class="absolute inset-0 bg-gradient-to-r from-[#828BF8] to-[#6B7BF8]" style="opacity: {{ opacity_value }};"></div>',
      '            <div class="absolute -top-2 -right-2 w-10 h-10 bg-[#FEF18C] border-[3px] border-black rotate-12 flex items-center justify-center font-black text-[10px] shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">ZAP!</div>',
      ...
      '            <div class="absolute -top-1 -right-1 w-8 h-8 bg-[#AAD6B8] border-[3px] border-black rotate-45"></div>',
      ...
      '            <div class="absolute -top-1 -right-1 w-7 h-7 bg-[#FEC4FF] border-[3px] border-black rotate-[20deg]"></div>',
      '          {% elsif rank_mod == 0 %}',
      '          <div class="border-[4px] border-black px-4 py-2.5 relative overflow-hidden">',
      ...
      '              ',
      '              {% if contributor.rank == 1 %}',
      '              <div class="rank-badge relative flex items-center justify-center w-9 h-9 bg-white border-[4px] border-black font-black text-sm" style="transform: rotate(-12deg);">',
      '                <span class="crown-icon absolute -top-5 text-2xl" style="filter: drop-shadow(2px 2px 0px rgba(0,0,0,0.3));">👑</span>',
      '                {{ contributor.rank }}',
      '              </div>',
      '              {% elsif contributor.rank == 2 %}',
      '              <div class="rank-badge flex items-center justify-center w-9 h-9 bg-white border-[3px] border-black font-black text-sm" style="transform: rotate(8deg);">',
      '                {{ contributor.rank }}',
      '              </div>',
      '              {% elsif contributor.rank == 3 %}',
      '              <div class="rank-badge flex items-center justify-center w-9 h-9 bg-white border-[3px] border-black font-black text-sm" style="transform: rotate(-5deg);">',
      '                {{ contributor.rank }}',
      '              </div>',
      '              {% else %}',
      '              <div class="flex items-center justify-center w-9 h-9 bg-white border-[3px] border-black font-bold text-sm">',
      '                {{ contributor.rank }}',
      '              </div>',
      '              {% endif %}',
      '              ',
      '              <div class="flex-1 min-w-0 flex items-center justify-between">',
      '                <div class="font-black text-base uppercase tracking-tight truncate" style="color: {{ data.text_color | default: \'#000000\' }}; text-shadow: 1px 1px 0px rgba(255,255,255,0.5), 2px 2px 0px rgba(0,0,0,0.15);">',
      '                  {{ contributor.name }}',
      '                </div>',
      '                <div class="font-black text-base whitespace-nowrap ml-3" style="color: {{ data.text_color | default: \'#000000\' }}; text-shadow: 1px 1px 0px rgba(255,255,255,0.5);">',
